...
function OverviewQuickButton({icon:Icon,label,onClick,disabled}:{icon:React.ComponentType<{className?:string}>;label:string;onClick?:()=>void;disabled?:boolean}){return <button type="button" onClick={onClick} disabled={disabled} className="flex min-h-[68px] flex-col items-center justify-center gap-2 rounded-lg border border-[#20364e] bg-[#102138] px-1.5 text-[11px] font-medium text-slate-200 transition hover:border-sky-500/40 hover:bg-[#132943] disabled:cursor-not-allowed disabled:opacity-35"><Icon className="size-5 text-slate-300"/>{label}</button>}
