...
export default function ServerPage(){
  ...
  const playerListSnapshot=useMemo(()=>{for(const row of [...logs].reverse()){const m=row.line.match(/There are \d+ of a max of \d+ players online:?\s*(.*)$/i);if(m)return{names:m[1].split(',').map(x=>x.trim()).filter(Boolean),createdAt:row.createdAt}}return{names:[] as string[],createdAt:null as string|null}},[logs]); const playerNames=playerListSnapshot.names
    ...
    <div className="flex min-h-svh">
      ...
      <section id="server-main-content" className="min-w-0 flex-1">
        ...
        <div className="mx-auto w-full max-w-[1540px] space-y-4 p-3 sm:p-4 lg:p-5">
          ...
          {section==='overview'&&<>
            <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_318px]">
              <div className="min-w-0 space-y-3">
                <DashboardCard className="overflow-hidden p-0">
                  <div className="flex flex-col gap-4 p-4 lg:flex-row lg:items-center lg:justify-between">
                    <div className="flex min-w-0 flex-col items-start gap-4 sm:flex-row sm:items-center">
                      {serverCoverUrl?<img src={serverCoverUrl} alt={`${server.name} kapak görseli`} className="size-[108px] shrink-0 rounded-xl border border-[#27415c] object-cover shadow-[0_12px_32px_rgba(0,0,0,.3)]"/>:<div className="grid size-[108px] shrink-0 place-items-center rounded-xl border border-[#27415c] bg-[radial-gradient(circle_at_30%_20%,rgba(56,189,248,.28),transparent_45%),#0c1b2c] text-sky-300"><ServerIcon className="size-12"/></div>}
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2"><h2 className="truncate text-2xl font-bold tracking-tight text-white">{server.name}</h2><span className={`inline-flex items-center gap-1.5 rounded-full border px-2 py-1 text-xs font-semibold ${processState==='ok'?'border-sky-500/30 bg-sky-500/10 text-sky-300':processState==='bad'?'border-red-500/30 bg-red-500/10 text-red-300':processState==='warn'?'border-amber-500/30 bg-amber-500/10 text-amber-300':'border-slate-600 bg-slate-700/20 text-slate-300'}`}><span className={`size-1.5 rounded-full ${processState==='ok'?'bg-cyan-400':processState==='bad'?'bg-red-400':processState==='warn'?'bg-amber-400':'bg-slate-500'}`}/>{processStatusLabel}</span></div>
                        <div className="mt-2 flex flex-wrap items-center gap-2 text-xs"><span className="rounded-md border border-[#253a52] bg-[#0b1928] px-2 py-1 text-slate-300">{server.loader} {server.mcVersion}</span><span className="rounded-md border border-[#253a52] bg-[#0b1928] px-2 py-1 text-slate-300">{node?.name??'Node doğrulanmadı'}</span></div>
                    ...
                    <div className="flex flex-wrap gap-2 lg:justify-end">{canStart&&<Button className="bg-sky-600 text-white hover:bg-sky-500" disabled={busy||running} onClick={()=>command('start')}><Play className="mr-2 size-4"/>Başlat</Button>}{canStop&&<Button className="border border-red-500/30 bg-red-600/80 text-white hover:bg-red-500" disabled={busy||!running} onClick={()=>command('stop')}><Square className="mr-2 size-4"/>Durdur</Button>}{canRestart&&<Button className="border border-blue-400/30 bg-blue-600/80 text-white hover:bg-blue-500" disabled={busy||!running} onClick={()=>command('restart')}><RefreshCw className="mr-2 size-4"/>Yeniden Başlat</Button>}</div>
                  </div>
                </DashboardCard>
                ...
                <div className="grid gap-3 lg:grid-cols-2">
                  <DashboardCard title="Oyuncu Aktivitesi" subtitle={metrics24h.length?'Gerçek sunucu metrik geçmişi':'Henüz telemetri örneği yok'}><TelemetryChart metrics={metrics24h} mode="players"/></DashboardCard>
                ...
                <div className="grid gap-3 lg:grid-cols-3">
                  ...
                  <DashboardCard title="Dikkat Gerektiren Noktalar" action={overviewAlerts.length?<span className="rounded-full border border-amber-500/30 bg-amber-500/10 px-2 py-1 text-xs text-amber-300">{overviewAlerts.length} uyarı</span>:undefined}>
                    {overviewAlerts.length?<div className="space-y-1.5">{overviewAlerts.slice(0,5).map((alert,index)=><div key={`${alert.title}-${index}`} className="flex items-start gap-2 rounded-lg border border-[#20354d] bg-[#0a1826] p-2.5"><AlertTriangle className={`mt-0.5 size-3.5 shrink-0 ${alert.tone==='critical'?'text-red-400':alert.tone==='warn'?'text-amber-400':'text-sky-400'}`}/><div className="min-w-0"><p className="text-[11px] font-medium text-slate-200">{alert.title}</p><p className="mt-0.5 line-clamp-2 text-xs text-slate-500">{alert.text}</p></div></div>)}</div>:<div className="rounded-lg border border-sky-500/20 bg-sky-500/[.05] p-4 text-center text-xs text-sky-300">Doğrulanmış aktif uyarı yok.</div>}
                  </DashboardCard>
                </div>

                <div className="grid gap-3 lg:grid-cols-3">
                  <DashboardCard title="Son İşlemler" action={<button className="text-xs text-sky-300 hover:text-sky-200" onClick={()=>setSection('logs')}>Tümünü Gör</button>}><div className="space-y-1">{(data?.operations??[]).slice(0,5).map(operation=><CompactDashboardRow key={operation.id} left={operation.operation} middle={operation.status} right={new Date(operation.createdAt).toLocaleString('tr-TR')} tone={operation.status==='failed'?'bad':operation.status==='completed'?'ok':'warn'}/>)}{!(data?.operations??[]).length&&<EmptyDashboardState text="Henüz işlem kaydı yok."/>}</div></DashboardCard>
                  <DashboardCard title="Son Hatalar ve Uyarılar" action={<button className="text-xs text-sky-300 hover:text-sky-200" onClick={()=>setSection('logs')}>Tümünü Gör</button>}><div className="space-y-1">{recentSystemEvents.map(row=>{const parsed=parseLog(row.line);return <CompactDashboardRow key={row.id} left={parsed.level} middle={parsed.message} right={new Date(row.createdAt).toLocaleString('tr-TR')} tone={parsed.level==='ERROR'?'bad':'warn'}/>})}{!recentSystemEvents.length&&<EmptyDashboardState text="Son günlüklerde önemli hata/uyarı yok."/>}</div></DashboardCard>
                  <DashboardCard title="Son Kayıp Eşya Kayıtları" action={canOpenSection('lost-items')?<button className="text-xs text-sky-300 hover:text-sky-200" onClick={()=>setSection('lost-items')}>Tümünü Gör</button>:undefined}><div className="space-y-1">{canSeeLostItems&&recentLostItems.map(item=><CompactDashboardRow key={item.id} left={item.playerName??'—'} middle={`${item.amount}× ${item.itemName}`} right={new Date(item.occurredAt).toLocaleString('tr-TR')} tone="info"/>)}{(!canSeeLostItems||!recentLostItems.length)&&<EmptyDashboardState text={canSeeLostItems?'Henüz kayıp eşya kaydı yok.':'Bu veriyi görüntüleme yetkiniz yok.'}/>}</div></DashboardCard>
                </div>
              </div>

              <aside className="space-y-3">
                <DashboardCard title="Sunucu Durumu">
                  <div className="flex items-center gap-3 border-b border-[#1b3047] pb-4"><span className={`size-5 rounded-full shadow-[0_0_18px_currentColor] ${processState==='ok'?'bg-cyan-400 text-sky-400':processState==='bad'?'bg-red-400 text-red-400':processState==='warn'?'bg-amber-400 text-amber-400':'bg-slate-500 text-slate-500'}`}/><div><p className={`text-base font-semibold ${processState==='ok'?'text-sky-300':processState==='bad'?'text-red-300':processState==='warn'?'text-amber-300':'text-slate-300'}`}>{processStatusLabel}</p><p className="text-xs text-slate-500">{processState==='ok'?'Process telemetrisi doğrulandı':running?'Canlı doğrulama tamamlanmadı':'Sunucu kapalı'}</p></div></div>
                  <div className="space-y-2 pt-4"><OverviewPropertyRow label="Minecraft" value={`${server.mcVersion} (${server.loader})`}/><OverviewPropertyRow label="Node" value={node?.name??'Doğrulanmadı'}/><OverviewPropertyRow label="Uptime" value={uptimeText}/><OverviewPropertyRow label="Process CPU" value={processCpu===null?'—':`%${Math.round(processCpu)}`}/><OverviewPropertyRow label="Process RAM" value={processMemoryUsed===null?'—':`${(processMemoryUsed/1024).toFixed(1)} GB`}/></div>
                </DashboardCard>

                <DashboardCard title="Bağlantı Bilgileri">
                  <button onClick={()=>{void navigator.clipboard.writeText(connectionAddress);setNotice('Sunucu adresi panoya kopyalandı.')}} className="flex w-full items-center gap-2 rounded-lg border border-[#23405e] bg-[#102138] px-3 py-3 text-left font-mono text-[11px] text-slate-200 hover:border-sky-500/50"><ServerIcon className="size-4 text-sky-300"/><span className="min-w-0 flex-1 truncate">{connectionAddress}</span><Copy className="size-4 text-slate-500"/></button>
                  <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-slate-500"><div>Sürüm <b className="block mt-1 text-slate-300">{server.mcVersion}</b></div><div>Loader <b className="block mt-1 text-slate-300">{server.loader}</b></div></div>
                </DashboardCard>

                <DashboardCard title="Hızlı İşlemler">
                  <div className="grid grid-cols-2 gap-2">{canOpenSection('console')&&<OverviewQuickButton icon={Terminal} label="Konsol" onClick={()=>setSection('console')}/>} {canOpenSection('players')&&<OverviewQuickButton icon={Users} label="Oyuncular" onClick={()=>setSection('players')}/>} {canOpenSection('files')&&<OverviewQuickButton icon={Folder} label="Dosyalar" onClick={()=>setSection('files')}/>} {canOpenSection('worlds')&&<OverviewQuickButton icon={Globe2} label="Dünyalar" onClick={()=>setSection('worlds')}/>} {canBackup&&<OverviewQuickButton icon={RotateCcw} label="Yedek Al" onClick={()=>backupAction('backup',{kind:'full',label:'overview-manual'})} disabled={busy}/>} {canOpenSection('settings')&&<OverviewQuickButton icon={Settings2} label="Ayarlar" onClick={()=>setSection('settings')}/>} {canOpenSection('lost-items')&&<OverviewQuickButton icon={Box} label="Kayıp Eşya" onClick={()=>setSection('lost-items')}/>} {canOpenSection('security')&&<OverviewQuickButton icon={ShieldCheck} label="Güvenlik" onClick={()=>setSection('security')}/>}</div>
                </DashboardCard>

                <DashboardCard title="Canlı Veri Kaynakları">
                  <div className="space-y-1"><OverviewStatusRow label="Panel API" state="ok" value="Bağlı"/><OverviewStatusRow label="Agent" state={heartbeatHealthy?'ok':onlineNode?'warn':'bad'} value={heartbeatHealthy?'Canlı':onlineNode?'Gecikiyor':'Doğrulanmadı'}/><OverviewStatusRow label="Process Metrics" state={metricFresh?'ok':running?'warn':'neutral'} value={metricFresh?'Canlı':running?'Bekleniyor':'Kapalı'}/><OverviewStatusRow label="server.properties" state={settingsVerified?'ok':'neutral'} value={settingsVerified?'Canlı':'Doğrulanmadı'}/></div>
                  <p className="mt-3 text-xs leading-4 text-slate-600">Bu ekranda sabit örnek değer kullanılmaz. Agent veya API doğrulayamıyorsa alan “— / Doğrulanmadı” olarak gösterilir.</p>
                </DashboardCard>
              </aside>
            </div>
          </>}
          ...
          {section==='players'&&<>
            <PageHeading title="Oyuncular" text="Oyuncuları görüntüleyin ve Minecraft komutlarıyla yönetin." right={<div className="flex items-center gap-2 text-[11px] text-slate-400"><Info className="size-4"/>Oyuncu sayısı agent tarafından düzenli ölçülür; isim listesi Yenile ile sunucudan alınır.<span className={`size-2 rounded-full ${running?'bg-cyan-400':'bg-slate-500'}`}/></div>}/>
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4"><MetricCard icon={Users} title="Anlık oyuncular" value={`${displayedPlayerCount} / ${actualMaxPlayers??'—'}`} detail={actualMaxPlayers?'Sunucu doluluk oranı':'Maksimum oyuncu değeri doğrulanamadı'} progress={actualMaxPlayers?Math.round(displayedPlayerCount/Math.max(1,actualMaxPlayers)*100):0}/><MetricCard icon={Clock3} title="Whitelist" value={actualWhitelist===null?'—':actualWhitelist?'Aktif':'Kapalı'} detail={actualWhitelist===null?'server.properties değeri yok':settingsVerified?'Canlı server.properties':'Son kaydedilen değer · canlı doğrulanmadı'}/><MetricCard icon={Ban} title="Yasaklı oyuncular" value="—" detail="Liste konsol komutuyla yönetilir"/><MetricCard icon={ShieldCheck} title="Anti-hile olayları" value={canSeeSecurity?String(antiCheatEvents.length):'—'} detail={canSeeSecurity?'Güvenlik olay kayıtlarından':'Güvenlik yetkisi yok'}/></div>
            <PanelCard className="p-0 overflow-hidden"><div className="flex flex-wrap items-center justify-between gap-2 p-4"><div><h3 className="text-sm font-semibold">Tüm oyuncular ({playerNames.length||displayedPlayerCount})</h3><p className="mt-1 text-xs text-slate-500">Son liste sorgusundaki oyuncuları görüntüleyin, filtreleyin ve yönetin. {playerListSnapshot.createdAt?`Son liste: ${new Date(playerListSnapshot.createdAt).toLocaleString('tr-TR')}`:'Henüz isim listesi alınmadı.'}</p></div><div className="flex flex-wrap gap-2"><div className="relative"><Search className="absolute left-2.5 top-2.5 size-3.5 text-slate-500"/><input value={playerName} onChange={e=>setPlayerName(e.target.value)} className="h-9 w-56 rounded-md border border-[#28445f] bg-[#091725] pl-8 pr-3 text-xs outline-none" placeholder="Oyuncu adı ara..."/></div><button onClick={()=>command('list-players')} disabled={!running||!canConsole} className="rounded-md bg-sky-600 px-3 text-xs font-semibold text-slate-950 disabled:opacity-40">Yenile</button></div></div><div className="overflow-x-auto"><table className="w-full min-w-[900px] text-xs"><thead className="border-y border-[#203a55] bg-[#0a1928] text-left text-slate-400"><tr><th className="p-3">Oyuncu</th><th className="p-3">Ping</th><th className="p-3">Rol</th><th className="p-3">Son giriş</th><th className="p-3">Güven puanı</th><th className="p-3">Hile riski</th><th className="p-3">Durum</th><th className="p-3">İşlemler</th></tr></thead><tbody>{playerNames.length?playerNames.filter(name=>!playerName||name.toLowerCase().includes(playerName.toLowerCase())).map((name,index)=><tr key={name} className="border-b border-[#1f3851]"><td className="p-3 font-semibold text-white"><span className="mr-2 inline-grid size-7 place-items-center rounded-md bg-[#0d2034] text-xs text-sky-300">{name.slice(0,2).toUpperCase()}</span>{name}</td><td className="p-3 text-slate-500">—</td><td className="p-3 text-slate-500">Rol verisi yok</td><td className="p-3 text-slate-400">Son liste sorgusunda çevrimiçi</td><td className="p-3 text-slate-500">—</td><td className="p-3"><span className="rounded-full bg-slate-500/10 px-2 py-1 text-slate-400">Telemetri yok</span></td><td className="p-3 text-sky-300">● Çevrimiçi</td><td className="p-3"><div className="flex gap-1.5">{canConsole&&<><Button size="sm" variant="outline" className="h-7 px-2 text-[11px]" onClick={()=>command('console',{line:`tell ${name} Merhaba`})}>Mesaj</Button><Button size="sm" variant="outline" className="h-7 px-2 text-[11px]" onClick={()=>command('console',{line:`tp ${name} 0 100 0`})}>TP</Button><Button size="sm" variant="outline" className="h-7 px-2 text-[11px]" onClick={()=>command('console',{line:`kick ${name}`})}>Kick</Button></>}</div></td></tr>):<tr><td colSpan={8} className="p-8 text-center text-slate-500">İsim listesi henüz alınmadı. Sunucu çalışırken Yenile'ye basın.</td></tr>}</tbody></table></div></PanelCard>
            <PanelCard title="Anti-hile olayları" subtitle="Son zamanlarda tespit edilen şüpheli hareketler ve sistem uyarıları." icon={ShieldCheck}>{canSeeSecurity?antiCheatEvents.slice(0,5).map((event,index)=><EventRow key={event.id??index} title={String(event.source??'anticheat')} text={String(event.event??'Anti-cheat olayı')} date={event.createdAt?new Date(event.createdAt).toLocaleString('tr-TR'):'—'}/>):<EmptyText text="Anti-cheat olayları için Güvenlik bölümü erişimi gerekir."/>}{canSeeSecurity&&antiCheatEvents.length===0&&<EmptyText text="Güvenlik Merkezi'nde anti-cheat olayı bulunmadı."/>}</PanelCard>
            <ServerPlayerModeration serverId={id} playerNames={playerNames} canManage={canConsole}/>
          </>}
...
function OverviewStatusRow({label,state,value}:{label:string;state:'ok'|'warn'|'bad'|'neutral';value:string}){const dot=state==='ok'?'bg-cyan-400':state==='warn'?'bg-amber-400':state==='bad'?'bg-red-400':'bg-slate-500';const text=state==='ok'?'text-sky-300':state==='warn'?'text-amber-300':state==='bad'?'text-red-300':'text-slate-400';return <div className="flex items-center gap-2 border-b border-[#172a3e] py-2 last:border-0"><span className={`size-2 shrink-0 rounded-full ${dot}`}/><span className="min-w-0 flex-1 truncate text-xs text-slate-400">{label}</span><span className={`text-xs font-medium ${text}`}>{value}</span></div>}
function OverviewQuickButton({icon:Icon,label,onClick,disabled}:{icon:React.ComponentType<{className?:string}>;label:string;onClick?:()=>void;disabled?:boolean}){return <button type="button" onClick={onClick} disabled={disabled} className="flex min-h-[72px] flex-col items-center justify-center gap-2 rounded-lg border border-[#20364e] bg-[#102138] px-2 text-xs font-medium text-slate-200 transition hover:border-sky-500/40 hover:bg-[#132943] disabled:cursor-not-allowed disabled:opacity-35"><Icon className="size-5 text-slate-300"/>{label}</button>}
...
function ResourceGauge({label,value,detail}:{label:string;value:number|null;detail:string}){const safe=value===null?0:Math.max(0,Math.min(100,value));const tone=value===null?'bg-slate-600':value>=90?'bg-red-400':value>=75?'bg-amber-400':'bg-cyan-400';return <div><div className="mb-1.5 flex items-center justify-between gap-3 text-[11px]"><span className="font-medium text-slate-300">{label}</span><span className="text-slate-500">{detail}</span></div><div className="h-2 overflow-hidden rounded-full bg-slate-800/80"><div className={`h-full rounded-full transition-all ${tone}`} style={{width:`${safe}%`}}/></div></div>}
...
function OverviewAlert({tone,title,text}:{tone:'critical'|'warn'|'info';title:string;text:string}){const style=tone==='critical'?'border-red-500/25 bg-red-500/[.07] text-red-200':tone==='warn'?'border-amber-500/25 bg-amber-500/[.07] text-amber-200':'border-blue-500/25 bg-blue-500/[.07] text-blue-200';return <div className={`flex items-start gap-3 rounded-lg border p-3 ${style}`}><AlertTriangle className="mt-0.5 size-4 shrink-0"/><div><p className="text-[11px] font-semibold">{title}</p><p className="mt-1 text-xs opacity-70">{text}</p></div></div>}
function PageHeading({title,text,right}:{title:string;text:string;right?:React.ReactNode}){return <div className="rounded-2xl border border-[#203a55] bg-[linear-gradient(120deg,rgba(13,29,45,.94),rgba(7,17,31,.9))] px-5 py-4 shadow-[0_18px_45px_rgba(0,0,0,.16)]"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[.22em] text-sky-400/75">BLOCKCTRL · SUNUCU YÖNETİMİ</p><h3 className="mt-1 text-[22px] font-bold tracking-tight text-white">{title}</h3><p className="mt-1 max-w-3xl text-[12px] leading-5 text-slate-400">{text}</p></div>{right}</div></div>}
function PanelCard({title,subtitle,icon:Icon,action,children,className=''}:{title?:string;subtitle?:string;icon?:React.ComponentType<{className?:string}>;action?:React.ReactNode;children?:React.ReactNode;className?:string}){return <section className={`rounded-2xl border border-[#203a55] bg-[linear-gradient(145deg,rgba(13,29,45,.96),rgba(7,18,30,.96))] p-4 shadow-[inset_0_1px_rgba(255,255,255,.018),0_14px_38px_rgba(0,0,0,.14)] ${className}`}>{(title||subtitle||action)&&<div className="mb-3 flex items-start justify-between gap-3"><div className="flex items-start gap-3">{Icon&&<div className="grid size-8 shrink-0 place-items-center rounded-lg bg-sky-500/10 text-sky-300"><Icon className="size-4.5"/></div>}<div>{title&&<h4 className="text-sm font-semibold text-white">{title}</h4>}{subtitle&&<p className="mt-0.5 text-[11px] text-slate-500">{subtitle}</p>}</div></div>{action}</div>}{children}</section>}
function MetricCard({icon:Icon,title,value,detail,progress,accent=false}:{icon:React.ComponentType<{className?:string}>;title:string;value:string;detail:string;progress?:number;accent?:boolean}){return <PanelCard className="min-h-[100px]"><div className="flex gap-3"><div className={`grid size-10 shrink-0 place-items-center rounded-lg ${accent?'bg-sky-500/12 text-sky-300':'bg-sky-500/[.08] text-sky-300'}`}><Icon className="size-5"/></div><div className="min-w-0 flex-1"><p className="text-[11px] text-slate-400">{title}</p><p className="mt-1 truncate text-[19px] font-semibold text-white">{value}</p><p className="mt-1 text-xs text-slate-500">{detail}</p>{progress!==undefined&&<div className="mt-2 h-1.5 rounded-full bg-slate-800"><div className="h-full rounded-full bg-cyan-400" style={{width:`${Math.max(0,Math.min(100,progress))}%`}}/></div>}</div></div></PanelCard>}
function CompactMetric({label,value,sub,good}:{label:string;value:string;sub:string;good?:boolean}){return <PanelCard className="min-h-[90px]"><p className="text-[11px] text-slate-500">{label}</p><p className={`mt-1.5 text-[17px] font-semibold ${good?'text-white':'text-white'}`}>{value}</p><p className="mt-1 text-xs text-slate-500">{sub}</p></PanelCard>}
function MiniStat({icon:Icon,label,value,sub,mono}:{icon:React.ComponentType<{className?:string}>;label:string;value:string;sub:string;mono?:boolean}){return <div className="flex gap-3 rounded-lg border border-[#1f3851] bg-black/10 p-3"><div className="grid size-9 shrink-0 place-items-center rounded-lg bg-sky-500/[.08] text-sky-300"><Icon className="size-4"/></div><div className="min-w-0"><p className="text-[11px] text-slate-500">{label}</p><p className={`mt-1 truncate text-sm font-semibold ${mono?'font-mono':''}`}>{value}</p><p className="mt-1 text-xs text-slate-500">{sub}</p></div></div>}
function QuickAction({icon:Icon,label,onClick,disabled}:{icon:React.ComponentType<{className?:string}>;label:string;onClick?:()=>void;disabled?:boolean}){return <button onClick={onClick} disabled={disabled} className="flex min-h-10 items-center gap-2 rounded-lg border border-[#203a55] bg-black/10 px-3 text-left text-[11px] font-medium text-slate-200 transition hover:bg-[#0d2034]/25 disabled:cursor-not-allowed disabled:opacity-35"><Icon className="size-4 text-slate-300"/>{label}</button>}
function HealthRow({label,value}:{label:string;value:number|null}){return <div className="flex items-center gap-3 border-b border-[#1d344c] py-2.5 last:border-0"><div className="min-w-0 flex-1"><p className="text-[11px] font-medium text-slate-200">{label}</p><div className="mt-1.5 h-1.5 rounded-full bg-slate-800">{value!==null&&<div className="h-full rounded-full bg-cyan-400" style={{width:`${Math.max(0,Math.min(100,value))}%`}}/>}</div></div><span className="text-[11px] font-semibold">{value===null?'—':`${value}%`}</span></div>}
function StatusLine({icon:Icon,label,sub,status}:{icon:React.ComponentType<{className?:string}>;label:string;sub:string;status:string}){const ok=/^(active|aktif|açık|ready|hazır|standard|strict|public|private)$/i.test(status);const bad=/^(off|kapalı|error|failed)$/i.test(status);return <div className="flex items-center gap-3 border-b border-[#1d344c] py-2 last:border-0"><div className="grid size-8 shrink-0 place-items-center rounded-lg bg-sky-500/[.08] text-sky-300"><Icon className="size-4"/></div><div className="min-w-0 flex-1"><p className="text-xs font-medium text-slate-200">{label}</p><p className="text-xs text-slate-500">{sub}</p></div><span className="flex items-center gap-1.5 text-[11px] text-slate-300"><span className={`size-1.5 rounded-full ${ok?'bg-cyan-400':bad?'bg-red-400':'bg-slate-500'}`}/>{status}</span></div>}
...
function OperationTable({rows}:{rows:Operation[]}){return <div className="space-y-1">{rows.length?rows.map(o=><div key={o.id} className="grid items-center gap-2 border-b border-[#1d344c] py-2 text-[11px] last:border-0 md:grid-cols-[90px_180px_1fr_150px]"><span className={`w-fit rounded-full px-2 py-1 ${o.status==='completed'?'bg-sky-500/10 text-sky-300':o.status==='failed'?'bg-red-500/10 text-red-300':'bg-amber-500/10 text-amber-300'}`}>{o.status}</span><code className="truncate text-slate-300">{o.operation}</code><span className="truncate text-slate-500">{o.message??'İşlem kaydı'}</span><span className="text-right text-slate-500">{new Date(o.createdAt).toLocaleString('tr-TR')}</span></div>):<EmptyText text="Henüz işlem kaydı yok."/>}</div>}
function Field({label,value,set}:{label:string;value:string;set:(v:string)=>void}){return <div className="space-y-1.5"><Label className="text-[11px] text-slate-300">{label}</Label><Input value={value} onChange={e=>set(e.target.value)} className="h-9 border-[#28445f] bg-[#091725] text-xs focus-visible:ring-sky-500/40"/></div>}
function SelectField({label,value,set,options,labels}:{label:string;value:string;set:(v:string)=>void;options:string[];labels?:Record<string,string>}){return <div className="space-y-1.5"><Label className="text-[11px] text-slate-300">{label}</Label><select value={value} onChange={e=>set(e.target.value)} className="h-9 w-full rounded-md border border-[#28445f] bg-[#091725] px-3 text-xs text-slate-200 outline-none focus:border-sky-500/50">{options.map(o=><option key={o} value={o}>{labels?.[o]??o}</option>)}</select></div>}
...
function SystemValue({label,value}:{label:string;value:string}){return <div className="flex items-center justify-between gap-4 border-b border-[#1d344c] px-4 py-3 odd:bg-white/[.012]"><span className="text-slate-500">{label}</span><span className="max-w-[60%] truncate text-right font-mono text-[11px] text-slate-300" title={value}>{value}</span></div>}
