            ...
            <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_318px]">
              ...
              <aside className="space-y-3">
                ...
                <DashboardCard title="Bağlantı Bilgileri">
                  <button onClick={()=>{void navigator.clipboard.writeText(connectionAddress);setNotice('Sunucu adresi panoya kopyalandı.')}} className="flex w-full items-center gap-2 rounded-lg border border-[#23405e] bg-[#102138] px-3 py-3 text-left font-mono text-xs text-slate-200 hover:border-sky-500/50"><ServerIcon className="size-4 text-sky-300"/><span className="min-w-0 flex-1 truncate">{connectionAddress}</span><Copy className="size-4 text-slate-500"/></button>
...
function CompactMetric({label,value,sub,good}:{label:string;value:string;sub:string;good?:boolean}){return <PanelCard className="min-h-[90px]"><p className="text-xs text-slate-500">{label}</p><p className={`mt-1.5 text-[17px] font-semibold ${good?'text-white':'text-white'}`}>{value}</p><p className="mt-1 text-xs text-slate-500">{sub}</p></PanelCard>}
