...
const cors={'Access-Control-Allow-Origin':'*','Access-Control-Allow-Methods':'GET,POST,OPTIONS','Access-Control-Allow-Headers':'Content-Type,Authorization,X-BlockCtrl-Server-Bridge','Cache-Control':'no-store'}
...
function rateLimited(result:LimitResult){return response({error:'Çok fazla deneme yapıldı. Lütfen daha sonra tekrar deneyin.',retryAfter:result.retryAfter},429,{'Retry-After':String(result.retryAfter)})}
...
async function getSiteForProvision(slug:string,serverId:string){
  ...
  return (await db.select().from(websites).where(eq(websites.id,matches[0].websiteId)).limit(1))[0]
}
function builderConfig(site:any){const data=site?.builderData&&typeof site.builderData==='object'?site.builderData as any:{};const cfg=data.auth&&typeof data.auth==='object'?data.auth:{};const registrationMode=['website','server','both','closed'].includes(String(cfg.registrationMode))?cfg.registrationMode:(cfg.allowRegistration===false?'closed':'website');const loginMode=['email','minecraft','both'].includes(String(cfg.loginMode))?cfg.loginMode:'email';return {enabled:cfg.enabled===true,allowRegistration:registrationMode==='website'||registrationMode==='both',registrationMode,loginMode,serverId:String(cfg.serverId||''),sessionDays:Math.max(1,Math.min(90,Number(cfg.sessionDays)||30)),defaultRole:String(cfg.defaultRole||'member'),serverBridgeEnabled:registrationMode==='server'||registrationMode==='both',loginPageSlug:String(cfg.loginPageSlug||'giris'),registerPageSlug:String(cfg.registerPageSlug||'kayit'),afterLoginPageSlug:String(cfg.afterLoginPageSlug||'hesabim')} as Config}
...
export async function GET(request:NextRequest){
  await ensurePanelSchema();const slug=String(request.nextUrl.searchParams.get('site')||'');const site=await getSite(slug);if(!site)return response({error:'Website bulunamadı.'},404);const cfg=await authConfig(site);if(!cfg.enabled)return response({error:'Website üye sistemi kapalı.'},403)
  const token=(request.headers.get('authorization')||'').replace(/^Bearer\s+/i,'').trim();if(!token)return response({user:null,config:cfg})
  ...
  if(!session)return response({user:null,config:cfg})
  ...
  if(!member||member.status!=='active')return response({user:null,config:cfg})
  await db.update(websiteMemberSessions).set({lastSeenAt:new Date()}).where(eq(websiteMemberSessions.id,session.id))
  return response({user:publicMember(member),config:cfg})
}


export async function POST(request:NextRequest){
  await ensurePanelSchema();const body=await request.json().catch(()=>({})) as Record<string,unknown>;const slug=String(body.site||'');const action=String(body.action||'');const provisionServerId=String(body.serverId||'');const site=action==='server-provision'?await getSiteForProvision(slug,provisionServerId):await getSite(slug);if(!site)return response({error:action==='server-provision'&&!slug?'Bu Minecraft sunucusuna bağlı tek ve etkin website bulunamadı.':'Website bulunamadı.'},action==='server-provision'?409:404);const cfg=await authConfig(site);if(!cfg.enabled)return response({error:'Website üye sistemi kapalı.'},403)
  if(action==='register'){
    if(!(cfg.registrationMode==='website'||cfg.registrationMode==='both')||!cfg.allowRegistration)return response({error:'Website üzerinden yeni üyelik kaydı kapalı.'},403)
    ...
    const registerIp=await consumeRateLimit(site.id,'register-ip',requestIp(request),8,60*60*1000);if(!registerIp.allowed)return rateLimited(registerIp)
    const registerIdentity=await consumeRateLimit(site.id,'register-identity',email||minecraftUsername||name||'anonymous',4,60*60*1000);if(!registerIdentity.allowed)return rateLimited(registerIdentity)
    const emailRequired=cfg.loginMode!=='minecraft',mcRequired=cfg.loginMode!=='email'
    if(name.length<2||password.length<8||(emailRequired&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))||(mcRequired&&!/^[A-Za-z0-9_]{3,32}$/.test(minecraftUsername)))return response({error:'Kayıt alanlarını seçilen giriş yöntemine göre eksiksiz doldurun; şifre en az 8 karakter olmalı.'},400)
    if(email){const exists=(await db.select({id:websiteMembers.id}).from(websiteMembers).where(and(eq(websiteMembers.websiteId,site.id),ilike(websiteMembers.email,email))).limit(1))[0];if(exists)return response({error:'Bu e-posta ile kayıt zaten var.'},409)}
    if(minecraftUsername){const exists=(await db.select({id:websiteMembers.id}).from(websiteMembers).where(and(eq(websiteMembers.websiteId,site.id),ilike(websiteMembers.minecraftUsername,minecraftUsername))).limit(1))[0];if(exists)return response({error:'Bu Minecraft kullanıcı adı zaten bağlı.'},409)}
    let member:any
    try{[member]=await db.insert(websiteMembers).values({websiteId:site.id,email:email||null,name,passwordHash:await hashPassword(password),minecraftUsername:minecraftUsername||null,serverId:cfg.serverId||null,authSource:'website',role:cfg.defaultRole,allowedPages:[]}).returning()}catch(error){if((error as {code?:string})?.code==='23505')return response({error:'Bu e-posta veya Minecraft kullanıcı adı zaten kayıtlı.'},409);throw error}
    ...
    return response({ok:true,token,user:publicMember({...member,lastLoginAt:new Date()}),redirect:`/${cfg.afterLoginPageSlug}`},201)
  }
  if(action==='server-provision'){
    if(!(cfg.registrationMode==='server'||cfg.registrationMode==='both')||!cfg.serverBridgeEnabled)return response({error:'Sunucu üzerinden kayıt bu website için kapalı.'},403)
    if(!bridgeAllowed(request))return response({error:'Sunucu bridge doğrulaması başarısız.'},401)
    ...
    if(!cfg.serverId||serverId!==cfg.serverId)return response({error:'Bu sunucu website kayıt kaynağı olarak seçilmemiş.'},403)
    if(!/^[A-Za-z0-9_]{3,32}$/.test(minecraftUsername)||password.length<8)return response({error:'Minecraft kullanıcı adı ve en az 8 karakter şifre gerekli.'},400)
    const provisionLimit=await consumeRateLimit(site.id,'server-provision',`${serverId}:${minecraftUsername}`,12,60*60*1000);if(!provisionLimit.allowed)return rateLimited(provisionLimit)
    const exists=(await db.select({id:websiteMembers.id}).from(websiteMembers).where(and(eq(websiteMembers.websiteId,site.id),ilike(websiteMembers.minecraftUsername,minecraftUsername))).limit(1))[0];if(exists)return response({error:'Bu Minecraft oyuncusu zaten kayıtlı.'},409)
    if(email){const emailExists=(await db.select({id:websiteMembers.id}).from(websiteMembers).where(and(eq(websiteMembers.websiteId,site.id),ilike(websiteMembers.email,email))).limit(1))[0];if(emailExists)return response({error:'Bu e-posta ile kayıt zaten var.'},409)}
    let member:any
    ...
    return response({ok:true,user:publicMember(member)},201)
  }
  if(action==='login'){
    const identifier=String(body.identifier||body.email||body.minecraftUsername||'').trim().slice(0,180),password=String(body.password||'');if(!identifier||!password)return response({error:'Giriş bilgileri eksik.'},400)
    const loginIp=await consumeRateLimit(site.id,'login-ip',requestIp(request),60,10*60*1000);if(!loginIp.allowed)return rateLimited(loginIp)
    const loginIdentity=await consumeRateLimit(site.id,'login-identity',identifier,8,10*60*1000);if(!loginIdentity.allowed)return rateLimited(loginIdentity)
    const member=await findMemberForLogin(site.id,identifier,cfg.loginMode)
    if(!member||member.status!=='active'||!(await verifyPassword(password,member.passwordHash)))return response({error:'Giriş bilgileri hatalı.'},401)
    ...
    return response({ok:true,token,user:publicMember({...member,lastLoginAt:now}),redirect:`/${cfg.afterLoginPageSlug}`})
  }
  if(action==='logout'){
    ...
    return response({ok:true,redirect:`/${cfg.loginPageSlug}`})
  }
  return response({error:'Geçersiz üye işlemi.'},400)
}