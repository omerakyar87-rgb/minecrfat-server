...
export function ControlPanel() {
  ...
  return <div className="min-h-svh bg-[#020805] font-sans text-foreground">
    ...
    <div className="lg:pl-64"><header className="sticky top-0 z-20 flex h-16 items-center border-b border-emerald-950/70 bg-[#020805]/90 px-4 backdrop-blur-xl md:px-6"><Button variant="ghost" size="icon" className="text-emerald-100 lg:hidden" onClick={()=>setMobileOpen(true)} aria-label="Menüyü aç"><Menu/></Button><div className="ml-auto flex items-center gap-3"><div className="hidden items-center gap-2 text-xs text-emerald-100/60 sm:flex"><span className="size-2 rounded-full bg-emerald-400 shadow-[0_0_10px_#4ade80]"/>Panel çevrimiçi</div><Button onClick={()=>setUtilityOpen(true)} aria-label="Yönetim araçlarını aç" className="gap-2 rounded-xl border border-emerald-300/30 bg-emerald-400/10 text-emerald-100 shadow-[0_0_24px_rgba(74,222,128,.12)] hover:bg-emerald-400/20"><Box className="size-4"/>Yönetim</Button><SupportCenter/></div></header><main id="worlds" className="p-4 md:p-6 lg:p-8"><InfrastructureManager/></main></div>
