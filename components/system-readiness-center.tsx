...
type Health={
  ...
  vercelWebsite:{status:CheckStatus;latencyMs:number|null;teamConfigured:boolean;error:string|null}
...
export function SystemReadinessCenter({manager=false}:{manager?:boolean}){
  ...
  return <div className="space-y-4">
    ...
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
      ...
      <StatusCard icon={Cloud} title="Website yayınlama" status={data.vercelWebsite.status} summary={data.vercelWebsite.status==='ok'?`Vercel API ${data.vercelWebsite.latencyMs??0} ms`:'Vercel yayın bağlantısı hazır değil'} detail={data.vercelWebsite.error??(data.vercelWebsite.teamConfigured?'Takım/proje bağlantısı doğrulandı.':'Takım/proje yapılandırması bekleniyor.')}/>
      <StatusCard icon={Globe2} title="Website runtime" status={data.websiteRuntime.status} summary={data.websiteRuntime.status==='ok'?'Public URL + server bridge hazır':'Website runtime ayarları eksik'} detail={data.websiteRuntime.error??'Yayınlanan siteler canlı BlockCtrl verisine erişebilir.'}/>
