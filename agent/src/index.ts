...
async function fullDiskInventory(id:string){
  ...
  const category=(path:string)=>{const top=path.split('/')[0].toLowerCase();if(top==='mods')return 'mods';if(top==='plugins')return 'plugins';if(top==='config')return 'config';if(top==='backups')return 'backups';if(top==='logs')return 'logs';if(top==='world'||top==='world_nether'||top==='world_the_end'||top.endsWith('_nether')||top.endsWith('_the_end'))return 'worlds';return 'server-files'}
